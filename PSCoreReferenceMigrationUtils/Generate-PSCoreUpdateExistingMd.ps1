...
#>
function Generate-PSCoreUpdateExistingMd()
{
    [CmdletBinding()]
    Param(
    ...
    $mamlfiles |`
        ForEach-Object {
            ...
            Write-Verbose "Starting to update existing markdown files for:"
            ...
            try{
                ...
                $excpErrmsg = "Unable to 'Update-MarkdownHelp' files for:`n`r" + `
                            "    Module     : $module `n`r" + `
                            "    Source     : $mamlfile `n`r" + `
                            "    Destination: $OutputFolder `n`r" + `
                            $(Format-ExceptionMessage ($_.Exception))
                Write-Error $excpErrmsg;
            }
        }
}